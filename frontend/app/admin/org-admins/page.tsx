...
export default function OrgAdminsPage() {
  ...
  const guardAndFetch = useCallback(async () => {
    ...
    try {
      const adminsRes = await apiFetch<{ data: OrgAdmin[] }>('/admin/org-admins');
                      ...
                      <td>
                        <div className="row-menu">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setOpenMenuFor(prev => prev === a.id ? null : a.id);
                            }}
                          >
                          ...
                          {openMenuFor === a.id && (
                            <div className="kebab-dropdown" ref={openMenuRef} onClick={e => e.stopPropagation()} role="menu">
                              <button type="button" className="kebab-item" onClick={() => { setOpenMenuFor(null); router.push(`/admin/org-admins/${a.id}/edit`); }}>
                                Edit
                              </button>
                              {a.orgs.length > 0 && (
                                <button type="button" className="kebab-item" onClick={() => { setOpenMenuFor(null); setPasswordTarget(a); }}>
                                  Reset Password
                                </button>
                              )}
                              <button type="button" className="kebab-item kebab-danger" onClick={() => { setOpenMenuFor(null); setDeleteTarget(a); setDeleteOrgId(a.orgs.length === 1 ? a.orgs[0].id : ''); }}>
                                Delete
                              </button>
                            </div>
                          )}
        ...
        <div className="modal-overlay" onClick={() => setDeleteTarget(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {deleteTarget.orgs.length > 1 ? (
              <>
                <div className="modal-body">
                  <strong>{deleteTarget.full_name || deleteTarget.email}</strong> belongs to multiple organizations. Choose an action:
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, margin: '12px 0' }}>
                  {deleteTarget.orgs.map(o => (
                    <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: deleteOrgId === o.id ? '#f0f4ff' : '#f9f9f7', border: `1px solid ${deleteOrgId === o.id ? '#c7d7ff' : '#e5e5e5'}`, borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
                      <input type="radio" name="deleteOrg" value={o.id} checked={deleteOrgId === o.id} onChange={() => setDeleteOrgId(o.id)} />
                      Remove from <strong style={{ marginLeft: 4 }}>{o.name}</strong>
                    </label>
                  ))}
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: deleteOrgId === '' ? '#fff5f5' : '#f9f9f7', border: `1px solid ${deleteOrgId === '' ? '#ffc7c7' : '#e5e5e5'}`, borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
                    <input type="radio" name="deleteOrg" value="" checked={deleteOrgId === ''} onChange={() => setDeleteOrgId('')} />
            ...
            <div className="modal-actions">
              <button className="btn" style={{ background: '#f5f4f1', color: '#1a1a1a', border: 'none' }} onClick={() => { setDeleteTarget(null); setDeleteOrgId(''); }}>Cancel</button>
              <button className="btn btn-danger" onClick={handleDelete} disabled={deleting}>
                {deleting ? 'Deleting…' : deleteOrgId ? 'Remove from Org' : 'Delete'}
              </button>
            </div>
          </div>
        </div>
      ...
      {passwordTarget && (
        <div className="modal-overlay" onClick={() => setPasswordTarget(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              A new temporary password will be generated for <strong>{passwordTarget.full_name || passwordTarget.email}</strong>. They will be required to set a new password on next login.
            </div>
            <div className="modal-actions">
      ...
      {resetTempPassword && (
        <div className="modal-overlay" onClick={() => setResetTempPassword(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">Share this temporary password with the admin securely. They will be prompted to set a new one on next login.</div>
            ...
            <p style={{ fontSize: 12, color: '#999', marginBottom: 12 }}>This will not be shown again.</p>
            <div className="modal-actions">
              ...
              <button className="btn" style={{ background: '#f5f4f1', color: '#1a1a1a', border: 'none' }} onClick={() => setResetTempPassword(null)}>
                Done
              </button>
            </div>
          </div>
