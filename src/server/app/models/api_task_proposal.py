...
class ApiTaskProposal(SQLModel, table=True):
    ...
    chat_session_id: UUID = Field(foreign_key="chat_sessions.id", ondelete="CASCADE", nullable=False, index=True)
    agent_task_id: UUID | None = Field(default=None, foreign_key="agent_tasks.id", ondelete="CASCADE")
    ...
    proposed_by: UUID | None = Field(default=None, foreign_key="users.id", ondelete="SET NULL")
    accepted_by: UUID | None = Field(default=None, foreign_key="users.id", ondelete="SET NULL")
    declined_by: UUID | None = Field(default=None, foreign_key="users.id", ondelete="SET NULL")
