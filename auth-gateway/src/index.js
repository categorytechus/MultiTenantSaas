...
const forwardToAuthService = (req, res) => {
  ...
  proxyReq.on("error", (err) => {
    ...
    res
      .status(502)
      .json({ error: "Bad Gateway", message: "Auth Service unavailable" });
  });
};
...
app.use("/api/admin", forwardToAuthService);
app.use("/api/permissions", forwardToAuthService);
...
async function submitTask(req, res, actionType, requiredPermission) {
  ...
  try {
    ...
    channel.publish("saas_exchange", routingKey, Buffer.from(msg), {
      persistent: true,
    });
...
app.post("/api/chat", jsonParser, (req, res) =>
  submitTask(req, res, "", "agents:run"),
);
app.post("/api/agents/start", jsonParser, (req, res) =>
  submitTask(req, res, "agents:create", "agents:create"),
);
app.post("/api/agents/:agentId/run", jsonParser, (req, res) =>
  submitTask(req, res, `agents:${req.params.agentId}`, "agents:run"),
);
